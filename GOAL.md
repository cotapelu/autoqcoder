...
Mandatory Reading: Trước khi thực hiện bất kỳ thay đổi nào, đọc phần liên quan
Checklists: Sử dụng các checklist như pre-flight before commit
Metrics: Theo dõi docs/AGENT_METRICS.md, docs/AGENT_PROFILE.md, docs/EVOLUTION.md
Autonomous Mode: Agent tự động chạy theo AUTO-CONTINUE workflow bên dưới
2. CORE DIRECTIVE
...
Scan toàn bộ codebase mỗi 2 giờ
Phát hiện violations: quality gate, anti-patterns, security, performance
Tạo plan cải thiện tự động, ưu tiên: Critical → High → Medium → Low
Thực thi từng bước với verification: Test → Implement → Refactor → Optimize → Audit → Verify
Commit với conventional commits khi completed
Log metrics to docs/AGENT_METRICS.md
Update profile weaknesses trong docs/AGENT_PROFILE.md
Update roadmap trong docs/EVOLUTION.md
2.2 Continuous Loop
...
Magic Constants: unnamed numbers/strings
Shotgun Surgery: >2 duplications across codebase
Circular Dependency: module cycles
Deep Inheritance: >2 levels hierarchy
Feature Envy: accessing other object's data >3x
N+1 Queries: loop with DB calls
Blocking I/O in async: sync operations in async context
O(n²) algorithms: nested loops over same data
Unbounded Cache: no TTL/eviction
Sync Rate Limiting: blocking rate limiter
...
Timeout: All I/O operations, default 10s (adjust per operation)
Circuit Breaker: Failure threshold 5, timeout 60s, half-open state
Bulkhead: Isolate resource pools (DB, HTTP) per service/tenant
Fallback: Cache/default/degraded mode when primary fails
Health Checks: As above
Graceful Shutdown: Handle SIGTERM/SIGINT, stop accepting new requests, finish in-flight, cleanup resources
4.5 Error Messages
...
Require API Compatibility section in project docs:
